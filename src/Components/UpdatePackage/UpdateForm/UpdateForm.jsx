...
import { INPUT_BENEFITS_TYPE, UPDATE_BENEFITS } from '../../apolloClient/package/mutation'
...
const UpdateForms = () => {
    ...
    const {data: getDataValue, loading} = useQuery(GET_DATA_DETAILS_PACKAGE, {
        variables: { _eq: idUpdate},
        onCompleted: (queryData) => {
            console.log(queryData)
            Object.keys(dataBenefits).forEach(key => {
                const findData = queryData.Benefit_type.find(item => item.RelationBenefit.name.toLowerCase() === key)
                const arrayDetails = JSON.parse(findData?.description || "[]")
                data[key] = arrayDetails
                setData({...data})
                // console.log(data)
    ...
    const [inputBenefitType, { loading: loadingAdd, error: errorAdd }] = useMutation(INPUT_BENEFITS_TYPE, {
        // onCompleted: () => {
        //     navigate('/package');
        // }
    });

    const [updateBenefitType, { loading: loadingForm, error }] = useMutation(UPDATE_BENEFITS, {
        // onCompleted: () => {
        //     navigate('/package');
        // }
    });
    ...
    const HandleClickAdd = () => {
        // console.log('test')
        /** Untuk ambil id checkbox */
        const benefit = JSON.parse(localStorage.getItem('benefit'))
        // console.log(benefit)

        Object.keys(benefit).forEach((key) => {
            /** Untuk mendapatkan value setiap benefit type */
            const findKey = getDataValue?.Benefit_type.find(item => item.RelationBenefit.name.toLowerCase() === key.toLowerCase())
            // console.log(findKey)
                ...
                <Row className="justify-content-md-center">
                    <Col xs={12} md={8}>
                        ...
                        <div className=" mt-3 mb-3 button">
                            ...
                            <Button variant="success" onClick={HandleClickAdd}>Update Packages</Button>
                        </div>
                    </Col>
                </Row>
            </Container>
